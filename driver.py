...
def KSL_Strang(t, X0, S0, V0):
    # Defining A for time t0 ,t_1/2 and t1
    A0, A_mid, A1 = A(t - 1), A(t - 1 / 2), A(t)

    # half K-step t0 -> t_1/2
    K0 = X0.dot(S0)
    K_mid = K_step(K0, V0, A_mid - A0)
    # QR decomposition
    X_mid1, S_mid1 = qr(K_mid)

    # half S-step t0 -> t_1/2
    S_mid2 = S_step(X_mid1, S_mid1, V0, A_mid - A0)

    # full L-step t0 -> t1
    L0 = S_mid2.dot(V0.T)
    L1 = L_step(L0, X_mid1, A1 - A0)
    # QR decomposition
    V1, S_mid3 = qr(L1)
    S_mid3 = S_mid3.T

    # half S-step t_1/2 -> t1
    S_mid4 = S_step(X_mid1, S_mid3, V1, A1 - A_mid)

    # half K-step t_1/2 -> t1
    K0 = X_mid1.dot(S_mid4)
    K1 = K_step(K0, V1, A1 - A_mid)
    # QR decomposition
    X1, S1 = qr(K_mid)
